...
export default {
  ...
  methods: {
    async getData() {
      try {
        const myRequest = new Request('test',{method: 'GET'});
  ...
  beforeCreate() {
    ...
    this.date_to_show = moment(monday).format('DD-MM-YYYY');
    this.columnsTable = []
    this.columnsTable.push('Semaine ' + this.date_to_show);
    this.columnsTable.push('Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche');
  },
  created() {
    this.getData();
  }
